...
Rune expressions are usually complex, which means they usually have one or more subexpressions. The appropriate syntax varies from rune to rune; after all, they’re used for different purposes. To see the syntax rules for a particular rune, consult the [rune reference](/docs/hoon/reference/rune/_index). Nevertheless, there are some general principles that hold of all rune expressions.
...
The Hoon syntax can be intimidating for the uninitiated, so it’s good to remember where you can look up expressions. The [Reference](/docs/hoon/reference/_index) section itself is a good place to find the materials that you need. These children sections are likely to be useful:

- The [Runes](/docs/hoon/reference/rune/_index) page will show you how to use any Hoon rune.
